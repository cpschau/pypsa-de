...
if __name__ == "__main__":
    ...
    # raw_temporal = [res["temporal aggregate"]["average_temperature"] for res in results]

    # # find first non-empty to copy lon/lat from
    # template = None
    # for da in raw_temporal:
    #     if da.size > 0:
    #         template = da
    #         break

    # if template is None:
    #     raise ValueError("All temporal aggregates are empty – cannot concatenate.")

    # # Temperature temporal aggregate: spatial distribution of sea water temperatures
    # cleaned = []
    # for res in results:
    #     da = res["temporal aggregate"]["average_temperature"]

    #     # drop all non-dimension coords (incl. spatial_ref if coord)
    #     da = da.reset_coords(drop=True)

    #     # DataArray-safe drop (only if present as coord/var)
